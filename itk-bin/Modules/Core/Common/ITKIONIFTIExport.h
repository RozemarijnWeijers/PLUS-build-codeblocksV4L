
#ifndef ITKIONIFTI_EXPORT_H
#define ITKIONIFTI_EXPORT_H

#ifdef ITK_STATIC
#  define ITKIONIFTI_EXPORT
#  define ITKIONIFTI_HIDDEN
#else
#  ifndef ITKIONIFTI_EXPORT
#    ifdef ITKIONIFTI_EXPORTS
        /* We are building this library */
#      define ITKIONIFTI_EXPORT __attribute__((visibility("default")))
#    else
        /* We are using this library */
#      define ITKIONIFTI_EXPORT __attribute__((visibility("default")))
#    endif
#  endif

#  ifndef ITKIONIFTI_HIDDEN
#    define ITKIONIFTI_HIDDEN __attribute__((visibility("hidden")))
#  endif
#endif

#ifndef ITKIONIFTI_DEPRECATED
#  define ITKIONIFTI_DEPRECATED __attribute__ ((__deprecated__))
#  define ITKIONIFTI_DEPRECATED_EXPORT ITKIONIFTI_EXPORT __attribute__ ((__deprecated__))
#  define ITKIONIFTI_DEPRECATED_NO_EXPORT ITKIONIFTI_HIDDEN __attribute__ ((__deprecated__))
#endif

#define DEFINE_NO_DEPRECATED 0
#if DEFINE_NO_DEPRECATED
# define ITKIONIFTI_NO_DEPRECATED
#endif

#endif
