...
${UNCRUSTIFYBIN} -c /home/snr/Support/Plus-build-codeblocks/itk/Utilities/Maintenance/uncrustify_itk.cfg -l CPP -f ${file} | sed 's/  *##  */##/g' | sed 's/#  */#/g' > ${file}_uncrustify
...
KWSTYLE_EXECUTABLE-NOTFOUND -xml /home/snr/Support/Plus-build-codeblocks/itk-bin/Utilities/KWStyle/ITK.kws.xml -v -o /home/snr/Support/Plus-build-codeblocks/itk/Utilities/KWStyle/ITKOverwrite.txt -gcc $file > ${KWERROS}
...
if [ -f ${KWERROS} ]; then
  ...
  if [ ${skipuncrustify} == "y" ]; then
    ${UNCRUSTIFYBIN} -c /home/snr/Support/Plus-build-codeblocks/itk/Utilities/Maintenance/uncrustify_itk.cfg -l CPP -f ${file} -o ${KWERROS}_skip_uncrustify
  fi
